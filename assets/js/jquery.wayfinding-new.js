...
// }
		...
		// Locate to nearest 
		function routeToNearest(destinationc, el) {
			var minDist = Infinity;
			var NearDest = '';
			var destinations = $('.' + destinationc);

			for (var i = 0; i < destinations.length; i++) {
				options.endpoint = $(destinations[i]).attr('id');
				var distance = getShortestRouteDistance();
				if (distance < minDist) {
					minDist = distance;
					NearDest = $(destinations[i]).attr('id');
				}
			}
			routeTo(NearDest, el);
		}
		...
		function routeTo(destination, el) {
			...
			if (startpoint !== destination) {
				...
				//display the destination room name
				$(el).trigger('wayfinding:wayto', [{ roomId: destination }]);
		...
		function addClass(passed) {
			...
			// $('.' + passed.preclass).addClass(passed.class);
			for(var preclass of passed.preclass.split(" ")){
				$('.' + preclass).addClass(passed.class);
			}
		}

		function removeClass(passed) {
			$('.' + passed.preclass).removeClass(passed.class);
		}
		...
		this.each(function () {
			...
			if (action && typeof (action) === 'string') {
				switch (action) {
					 ...
					 * @public
					 * @memberOf wayfinding
					 * @example $('target').wayfinding('routeToNearest', 'doorID');
					 */
					case 'routeToNearest':
						// call method
						//clearHighlightClass();
						routeToNearest(passed, obj);
						break;
					 ...
					 * @public
					 * @memberOf wayfinding
					 * @example $('target').wayfinding('addClass', 'doorID');
					 */
					case 'addClass':
						// call method
						clearHighlightClass();
